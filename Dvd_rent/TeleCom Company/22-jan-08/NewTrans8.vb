Imports V6Str = Microsoft.VisualBasic.Strings
Public Class NewTrans
    ...
    Private Sub NewTrans_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Me.AukDatabaseDataSet.Mob_Cards.Card_CategoryColumn.DefaultValue = 0
        Me.AukDatabaseDataSet.Mob_Cards.Cards_InStockColumn.DefaultValue = 0
        Me.AukDatabaseDataSet.Mob_Cards.Card_Benefit_PercentColumn.DefaultValue = 0
        Me.AukDatabaseDataSet.Mob_Cards.Cards_SoldColumn.DefaultValue = 0
        Me.AukDatabaseDataSet.Mob_Cards.S_DueCards_AmountColumn.DefaultValue = 0
        Me.AukDatabaseDataSet.Mob_Cards.S_DueCardsColumn.DefaultValue = 0
        Me.AukDatabaseDataSet.Mob_Cards.S_Total_BenefitColumn.DefaultValue = 0
        Me.AukDatabaseDataSet.Mob_Cards.S_Total_DiscountColumn.DefaultValue = 0
        Me.AukDatabaseDataSet.Mob_Cards.S_Total_Sold_CountColumn.DefaultValue = 0
        Me.AukDatabaseDataSet.Mob_Cards.Total_BenefitColumn.DefaultValue = 0
        Me.AukDatabaseDataSet.Mob_Com.BENEFIT_Column.DefaultValue = 0
        Me.AukDatabaseDataSet.Mob_Com.Date_TimeColumn.DefaultValue = DBNull.Value
        Me.AukDatabaseDataSet.Mob_Com.Digits_FindColumn.DefaultValue = 0
        Me.AukDatabaseDataSet.Mob_Com.Discount_Column.DefaultValue = 0
        Me.AukDatabaseDataSet.Mob_Com.Digits_FindColumn.DefaultValue = 0

        Me.vIDEOhELP_ITEM.Visible = CSOFT.VideoHelpItemFolder("VHelp", "MRecharge")
        ToolStripSeparator25.Visible = Me.vIDEOhELP_ITEM.Visible
        NeedTO_INFOR_FONT_CHG = True
        ' check for time trial stop
        '----------
        'If TVer = True Then
...
nextwork:
        ...
        Else
            Load_Sum = True
        End If
        Refresh_Stock_info_tab(NDate) 'took stock information is ena
    ...
    Public Sub Refresh_Stock_info_tab(ByVal Date1 As Date, Optional ByVal CheckNeed As Boolean = False)
...
startRe:    SFC("Date_info")
            STC(Date1)
            DTC("d")
            AukF.Db_LoadN(Me.AukDatabaseDataSet.DaY_Sold_Info, -1, "", True)
            Try
                If AukF.BindFind(Me.DaY_Sold_InfoBindingSource, "Date_info", Date1, True) = False Then
        ...
        End If
    End Sub
    Private Function INDEX_DATACHAGE(ByVal D As DataGridView, ByVal INDEX1 As Integer) 'before datarow text on now text
    ...
    Public Sub BLANCE()
        ...
        If Status <> CSold AndAlso UCase(CellField) <> "CARD SELLING" Then
        ...
        ElseIf Status = CSold OrElse UCase(CellField) = "CARD SELLING" Or UCase(CellField) = "CARDSELLING" Then 'cards information
            Me.BLANCE_LABEL.Tag = "c"
            Dim Rx1() As DataRow
            TCom = AukF.GridT(Drv, 3).ToString 'geting com id
            CardCate = Val(AukF.GridT(Drv, 4, -1, -1).ToString) 'geting the value of amount field
            If tcom Is Nothing OrElse CardCate = -1 Then GoTo innotfound
...
innotfound:     Me.BLANCE_LABEL.Text = "CARD INFORMATION NOT FOUND"
        ...
        If iNFOR.Visible = True Then
            ...
            If NeedTO_INFOR_FONT_CHG = True Then
                ...
                axD = Me.iNFOR.Font.ToString
                If axD <> My.Settings.FONT_INFO_MOB.ToString Then
    ...
    Public Function CELL_NUMB_GET() As String
        On Error Resume Next
        Cell = Replace(CStr(AukF.GridT(Drv, 2)), " ", "") 'MOB NUM
    ...
    Private Sub New_TransDataGridView_CellBeginEdit(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellCancelEventArgs) Handles New_TransDataGridView.CellBeginEdit
        On Error Resume Next
        If Me.New_TransDataGridView.CurrentRow.Index > -1 Then
            If e.ColumnIndex = 4 Then
                Before_Bal = Val(AukF.GridT(sender, 4, -1, 0)) 'so that i may minus this and add or minus the new amount
                BCardCate = Val(Before_Bal)
        ...
        If CurId <> "" Then
            ...
            If Rx2 IsNot Nothing Then 'THEN WE TRY TO SELL CARDS
                ...
                If Rx1 IsNot Nothing Then
                    ...
                    Dim R As DataRow
                    Me.RECHARGE_NUMBER(CSold, SoftIn.Boolean_get(TransR.Item("DUE_CARDS").ToString)) 'make current card sold,if before no efftect
                    'rx1=mob_CardsRow
                    R = Rx1.GetValue(0)
    ...
    Private Sub New_TransDataGridView_CellEndEdit(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles New_TransDataGridView.CellEndEdit
        ...
        If e.ColumnIndex = 3 Then 'CATEGORY ID EDIT
            ...
            Sn1 = AukF.GridT(sender, 2) 'cell num
            If Sn1 = "" Then
                sender(2, e.RowIndex).value = "CARD SELLING"
            Else
                Sx1 = V6Str.Left(Sn1, 3).ToString 'cellnum 3 digit
                If UCase(Sx1) <> "CAR" Then
                    If AukF.BindFind(Me.MobComBindingSource, "DIGITS", Sx1) = True Then
                        sender(3, e.RowIndex).value = Me.AukDatabaseDataSet.Mob_Com(ComRow).Item("Category_ID").ToString 'category by digit of cell num
                    End If
                End If
            End If
        End If
        If e.ColumnIndex = 1 Then 'CID EDITED
            ...
            If Sn1 <> "" AndAlso IsNumeric(Sn1) = False Then
                If LCase(V6Str.Left(Sn1, 1)) = "c" Then
                    Num = V6Str.Right(Sn1, Len(Sn1) - 1)
                    ...
                    End If
                End If
            ElseIf IsNumeric(Sn1) = True And Sn1 <> "" Then
                SFC("Client_ID")
                DTC("n")
                STC(Sn1)
                SnX("=")
            ...
            If MClients.Rows.Count = 1 Then
                ...
                If CrdSell = False Then
                    sender(2, e.RowIndex).value = MClients.Rows(0).Item(0).ToString 'cell number
                    c_name = UCase(c_name & "(" & Sn1 & ")")
                Else
                    c_name = UCase(c_name & "(" & Num & ")")
                    sender(2, e.RowIndex).value = "CARD SELLING"
                End If
                Sx1 = Mid(MClients.Rows(0).Item(0).ToString, 1, 3).ToString 'cell num 3 digit
        ...
        If e.ColumnIndex = 4 Then 'AMOUNT EDITED
            ...
            ElseIf (mxd1 = Snd) OrElse (mxd1 = DueStr) Then
        ...
        'extra on
        If My.Settings.EXTRA_M_TR = True OrElse (My.Settings.EXTRA_EFF2_MOB.ToString <> "" OrElse My.Settings.EXTRA_EFF2_MOB.ToString <> "NO EFFECT,0") Then 'extra on
            If (AukF.GridT(Drv, 8).ToString = CSold OrElse AukF.GridT(Drv, 2).ToString = "CARD SELLING") AndAlso (My.Settings.EXTRA_EFF2_MOB.ToString <> "" OrElse My.Settings.EXTRA_EFF2_MOB.ToString <> "NO EFFECT,0") Then 'getin gfrom status/celll num  & for Extra on Cards
                ...
                If Nm1 <> "" Then
                    ...
                    If Index1 = 0 Then GoTo closeafter
                    If Index1 = 1 AndAlso Amt <= 50 Then Ext = 1 : GoTo WORKSX
                    If Index1 = 2 AndAlso Amt <= 30 Then Ext = 1 : GoTo WORKSX
                    If Index1 = 3 AndAlso Amt <= 20 Then Ext = 1 : GoTo WORKSX
                    If Index1 = 4 AndAlso Amt <= 50 Then Ext = 2 : GoTo WORKSX
                    If Index1 = 5 AndAlso Amt <= 30 Then Ext = 2 : GoTo WORKSX
                    If Index1 = 6 AndAlso Amt <= 20 Then Ext = 2 : GoTo WORKSX
WORKSX:             If DRow > -1 AndAlso Ext > -1 Then Drv(5, DRow).Value = Ext Else Drv(5, DRow).Value = 0
                    'MsgBox(My.Settings.EXTRA_EFF2_MOB.ToString)
                End If
            Else 'eslse to goto recharge extra effect.
                If My.Settings.EXTRA_M_TR = True Then
                    If Amt < Val(My.Settings.EXTRA_LESS) AndAlso My.Settings.EXTRA_M_TR = True Then 'effect by eftra commands
        ...
        End If
closeafter:
        If e.ColumnIndex = 1 Then ShowDueClient(AukF.GridT(sender), True)
        If e.ColumnIndex = 2 Then ShowDueClient(AukF.GridT(sender), False)
        If e.ColumnIndex = 9 OrElse e.ColumnIndex = 4 OrElse e.ColumnIndex = 5 OrElse e.ColumnIndex = 6 Then 'eFFECT ON CURRENT BENEFT AFTER EDIT DISCOUNT
            Me.Set_Benefit_OnCurrent(False)
        End If
        If My.Settings.DIRECT_SAVE = True Then Me.SAVE_MOBS()
    ...
    End Sub
    Public Sub Set_Benefit_OnCurrent(Optional ByVal SaveDb As Boolean = True)
...
workon:
        ...
        'Me.New_TransDataGridView.EndEdit()
        Me.New_TransBindingSource.EndEdit()
        If SaveDb = True AndAlso My.Settings.DIRECT_SAVE = True Then Me.SAVE_MOBS()
    ...
    Public Sub Extent_Fields_readOnly()
        If Me.D_Benefit_Fields.ReadOnly = False Then Me.D_Benefit_Fields.ReadOnly = True
        If Me.d_Date_Field.ReadOnly = False Then Me.d_Date_Field.ReadOnly = True
        If Me.Dstatus_Field.ReadOnly = False Then Me.Dstatus_Field.ReadOnly = True
        If Me.D_SendTime_Field.ReadOnly = False Then Me.D_SendTime_Field.ReadOnly = True
        If Me.Due_Cards_Field.ReadOnly = False Then Me.Due_Cards_Field.ReadOnly = True
    End Sub
...
DOWOR:  Me.MonthCalendar1.Visible = False
        ...
        Try
            If IsNumeric(nUM) = False Then xca = nUM
            Me.New_TransDataGridView.ReadOnly = False
            If xca = "" Then
    ...
    Private Sub ToolStripLabel2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripLabel2.ButtonClick
        Dim DMD1 As Date
        Try
...
DOWORKD:
        ...
        ExpreC("*l", "l*")
        BracketsSql = False
        aBSql = AukSql.AukSql_Main().ToString
        'MsgBox(BSql)
        SFC("", "", "STATUS")
        STC(Str1, Str1, "SEND BUT NOT PAID YET")
        BracketsSql = False
        bBSql = AukSql.AukSql_Main().ToString
        SFC("duE_cARDS")
        STC("TRUE")
        DTC("B")
        BracketsSql = False
        CBSql = AukSql.AukSql_Main().ToString
        'MsgBox(BSql)
        a = "(" & aBSql & ") And (" & bBSql & " OR " & CBSql & ")"
        'MsgBox(a)
    ...
    Private Sub Timer1_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Timer1.Tick
        ...
        End If
        'time trail stop
        ''tried to stop on exparied time
        'If TVer = True Then
        '    Dim P As Decimal
        ...
        Else
            ...
            End If
        End If
        If Me.DueInfo.Visible = False AndAlso My.Settings.ShowDue_Mob = True Then
            Me.DueInfo.Visible = True
        ElseIf Me.DueInfo.Visible = True AndAlso My.Settings.ShowDue_Mob = False Then
            Me.DueInfo.Visible = False
        End If
        If Me.Due_Cards_Field.ReadOnly = False Then
            Extent_Fields_readOnly()
    ...
    Private Sub ToolStripButton3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripButton3.Click, SEARCHQUERYMANAGERToolStripMenuItem.Click, ToolStripMenuItem9.Click
        ...
        DVDRENT.ShowQr_Product(New_TransDataGridView, New_TransBindingSource)
    End Sub
    ...
    Public Sub RECHARGE_NUMBER(Optional ByVal S As String = "SEND", Optional ByVal CardsDue As Boolean = False)
        ...
        ElseIf STR1 = DueStr Or STR1 = Snd Then 'due/snd is same and there is not extra features,due is written for only save 
            ...
            ElseIf STR1 = DueStr And S = Snd Then 'if due to snd ,means due to paid then
                ...
                If My.Settings.TOOK_WHOL_INFO_MOB = True Then
                    Dim DN1, dN2 As Date
                    DN1 = CDate(AukF.GridT(Drv, 7).ToString) 'geting grid date
            ...
            End If
        End If
        m = AukF.GridT(Me.New_TransDataGridView, 8)
        If S = CSold And m = S Then
            If DRow > -1 Then Drv(10, DRow).Value = CStr(TimeOfDay)
            ...
            Else
                'by paying due
                Dim BeforeWas As Boolean
                BeforeWas = CBool(AukF.GridT(Drv, 12).ToString)
                If My.Settings.TOOK_WHOL_INFO_MOB = True Then
                    If BeforeWas = True Then 'if before due=true then effecton
                        Dim DN1, dN2 As Date
                        DN1 = CDate(AukF.GridT(Drv, 7).ToString) 'geting grid date
                        If Me.AukDatabaseDataSet.DaY_Sold_Info.Rows.Count = 1 Then
                            dN2 = CDate(Me.AukDatabaseDataSet.DaY_Sold_Info(0).Item("Date_Info").ToString)
...
BENEFIT_WORK:
    ...
    Private Sub Check_Due_Loss_BenefitC()
        ...
        If My.Settings.REF_DUE_BENEFIT_MOB = True Then
            If Me.BenefitTotalLabel.Visible = False Then
                Me.BenefitTotalLabel.Visible = True
                Me.BENEFIT_TOTAL_LABEL.Visible = True
                Me.DueFilterSplitBut.Text = "DUE:"
                Me.LOSS_LABEL.Visible = True
                Me.LossToalLabel.Visible = True
            End If
            ...
            If ADSRC.FIND_FROM_DATATABLE_INDEX(Me.AukDatabaseDataSet.Mob_Com, Sq) = False Then
                ...
                Me.BenefitTotalLabel.ForeColor = Color.Black
                Me.BenefitTotalLabel.BackColor = Color.White
                Benefit_p = 0
                Exit Sub
            Else
                Benefit_p = CDec(Val(RowX.Item("Benefit_Percent").ToString))
            End If
            Me.BenefitTotalLabel.Text = UCase(Sq) & " BENEFIT:"
            Me.BenefitTotalLabel.ForeColor = Color.Blue
            Me.BenefitTotalLabel.BackColor = Color.Black
            Me.DUE_TOTAL_LABEL.Text = Format(Val(ADSRC.SUMMARY_NONEXE_CUR_TABLE(Me.AukDatabaseDataSet.New_Trans, "(CATAGORY_ID='" & Sq & "') and ((Status='SEND BUT NOT PAID YET') or (Status='" & CSold & "' and DUE_CARDs=True))", "Sum", 4)), "0.##") '", "SUM", 4)), "0.##") 'AMOUNT
            BeneFit_ = CDec(Val(ADSRC.SUMMARY_NONEXE_CUR_TABLE(Me.AukDatabaseDataSet.New_Trans, "CATAGORY_ID='" & Sq & "' and (Status='SEND' or Status='" & CSold & "') and Due_Cards=False", "SUM", 11))) '11=benefit
            Me.BENEFIT_TOTAL_LABEL.Text = Format(BeneFit_, "0.##")
            ...
            Else
                ...
                Me.LOSS_LABEL.ForeColor = Color.Black
            End If
        Else 'else refresh false
            If Me.BenefitTotalLabel.Visible = True Then
                Me.BenefitTotalLabel.Visible = False
                Me.BENEFIT_TOTAL_LABEL.Visible = False
                Me.DueFilterSplitBut.Text = "DUE FILTER"
                Me.DUE_TOTAL_LABEL.Text = "REFRESH BENEFIT"
                Me.LOSS_LABEL.Visible = False
                Me.LossToalLabel.Visible = False
            End If
        End If
    End Sub
    Public Function MINUS_BLANCE(Optional ByVal Amt As Double = -1)
        ...
        If AukF.BindFind(Me.MobComBindingSource, "CATEGORY_ID", CTA) = True Then
            ...
            If (BEBAL - Amt) < 0 Then
                MsgBox("CURRENT(" & Amt & ") AMOUNT IS CROSSING YOUR (" & Me.AukDatabaseDataSet.Mob_Com(ComRow).Item("Company").ToString & ") BALANCE,WHICH IS " & BEBAL & ".", MsgBoxStyle.Exclamation)
                If BEBAL > 0 Then
...
b1:         If New_TransDataGridView.CurrentRow Is Nothing Then Exit Sub
    ...
    Private Sub ToolStripButton4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MOBILECOMPANYToolStripMenuItem.Click, ToolStripButton4.Click
        On Error Resume Next
        FrmL(MCompany)
    End Sub
...
DOWORKD:
        ...
        If AukF.MsgTrU("LOAD CARDS DUE?", True) = True Then
            SFC("Date_Business", "Due_Cards")
            STC(CDate(Now.Date), "TRUE")
            DTC("d", "B")
            SnX("=")
            AukF.Db_LoadN(Me.AukDatabaseDataSet.New_Trans, -1, "", False)
        End If
    End Sub
    Private Sub DUEbY_MONTH(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripMenuItem18.Click
        ...
        Exit Sub
DOWORKD:
        'Sql = "MONTHS_BETWEEN(" & CDate((1 & "/" & Now.Date.Month & "/" & Now.Year) & ",DATE_BUSINESS" & ")=0"
        ...
        If AukF.MsgTrU("LOAD CARDS DUE?", True) = True Then
            SFC("Date_Business", "Date_Business", "Due_Cards")
            SumC("month", "Year")
            STC(Val(Now.Month), Val(Now.Year), "TRUE")
            DTC("n", "n", "b")
            SnX("=")
            AukF.Db_LoadN(Me.AukDatabaseDataSet.New_Trans, -1, "", False)
        End If
...
DOWORKD:
    ...
    Public Sub Get_All_Benefit_in_their_Field() 'Sending Benefit
        ...
        If Me.AukDatabaseDataSet.New_Trans.Rows.Count < (Val(My.Settings.MaximumData_load)) Then
            ...
            For k As Integer = 0 To Me.AukDatabaseDataSet.New_Trans.Rows.Count - 1
                ...
                CardDueB = CBool(Rm1.Item("Due_Cards").ToString)
                'getin complete
                If sT = Snd Or sT = CSold Then
                    ...
                    ElseIf sT = CSold Then
                        'getting tCom by CID
                        If CardDueB = False Then 'if due card  is false
                            Dim Rx1() As DataRow
                            TCom = CiD  'geting com id
                            CardCate = Amt 'geting the value of amount field
                            If TCom IsNot Nothing Then Rx1 = SoftIn.Rows_Select(Me.AukDatabaseDataSet.Mob_Cards, "category_id='" & TCom & "' and Card_Category=" & CardCate)
                            If Rx1 IsNot Nothing Then
                                RowX = ArV2(Rx1, 0) 'a row of mob cards
                                MBp = Val(RowX.Item("CArd_Benefit_Percent").ToString)
                            Else
                                MBp = 0
                            End If
                        ElseIf CardDueB = True Then 'card due=true 
                            MBp = 0
    ...
    Public Sub DUE_LOAD_BY_DATE_IN_A_FORMAT(ByVal C As String, ByVal CTRLx As Boolean)
        ...
        Try
            If IsNumeric(W2) = True Then
                D1 = (CDate((Now.Date)).AddDays(Val(W2)))
            Else
                D1 = CDate(W2)
            End If
        ...
        S1 = AukSql.AukSql_Main("*", "NEW_TRANS")
        SFC("DATE_BUSINESS")
        STC(D1)
        DTC("D")
        S2 = AukSql.AukSql_Main("*", "NEW_TRANS")
        SFC("Status", "Due_Cards")
        STC(DueStr, "True")
        DTC("", "b")
        SJOIN = "Or"
        BracketsSql = False
        S4 = AukSql.AukSql_Main("*", "NEW_TRANS")
        S3 = "(" & S1 & ") AND (" & S2 & ") AND ( " & s4 & ")"
        'S = AukSql.AukSql_Main("*", "NEW_TRANS")
        AukF.Db_LoadQ(Me.AukDatabaseDataSet.New_Trans, S3, -1, False)
        'MsgBox(BSql)
        AukF.BindFilter_BSql(Me.New_TransBindingSource, BSql)
    ...
    Private Sub NewTrans_Activated(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Activated
        ...
        If StockI_mob.Visible = True Then
            Me.Refresh_Stock_info_tab(NDate) : Me.NeedToStockRe = False
        End If
    End Sub
...
DOWORKD:
        ...
        AukSql.AukSql_Main("*", "New_Trans")
        x3 = BSql
        x4 = SoftIn.GetFunction_QrText(Me.AukDatabaseDataSet.New_Trans, "Due_Cards", "True", 0, "=", False, ";", "%", False)
        a1 = "(" & x1 & ") OR (  (" & X2 & " OR " & x4 & " )  AND (" & x3 & ")   ) OR ([STATUS] IS NULL OR  [STATUS] ='')"
        AukF.Db_LoadQ(Me.AukDatabaseDataSet.New_Trans, a1, -1, True)
    End Sub
    Public Sub ShowDueClient(ByVal Number As String, Optional ByVal ID As Boolean = False)
        If My.Settings.ShowDue_Mob = True AndAlso Me.DueInfo.Visible = True Then
            If Trim(Number) = "" Then GoTo normal
            Dim T1 As DataTable
            Dim DueAmt As Decimal
            ...
            If ID = True Then
                ...
                STC(Number)
                ExpreC("f2")
                Str1 = "CLIENTS ID : " & Number
            Else
                SFC("Mobile_Number")
                STC(Number)
                ExpreC("f2")
                Str1 = "CELL NUMBER : " & Number
            End If
            If Microsoft.VisualBasic.Strings.Left(LCase(Number), 4) = "card" Then
                Str1 = "TOTAL DUE RECORDS IN CARDS SELLING IS "
                CrdSell = True
            End If
            a = "([Status]='" & DueStr & "' OR Due_Cards=True)"
            SFC("", "k")
            STC("", a)
            ExpreC("", "q")
            T1 = ADSRC.AdvSummSys_RTable(Me.AukDatabaseDataSet.New_Trans, "Amount,Amount", "SUM,coUNT")
            If T1 IsNot Nothing AndAlso T1.Rows.Count > 0 Then
                Count = Val(T1.Rows(0).Item(1).ToString)
                DueAmt = Val(T1.Rows(0).Item(0).ToString)
            Else
                Count = 0
                DueAmt = 0
            End If
            If CrdSell = False Then
                If Count > 0 Then
                    Me.DueInfo.BackColor = Color.Red
                    Me.DueInfo.ForeColor = Color.White
                    Me.DueInfo.Text = Str1 & " HAS " & Count & " DUE RECORDS . HIS/HER TOTAL DUE IS " & DueAmt & "."
                    Me.DueInfo.Tag = Number
                    Me.DueInfo.AccessibleDescription = ID
                Else
normal:             Me.DueInfo.BackColor = Color.DeepSkyBlue
                    Me.DueInfo.ForeColor = Color.Black
                    Me.DueInfo.Text = "CURRENT CUSTOMER HAS NO DUE RECORD(S)."
                    Me.DueInfo.Tag = ""
                    Me.DueInfo.AccessibleDescription = ""
                    Exit Sub
                End If
            Else
                If Count > 0 Then
                    Me.DueInfo.BackColor = Color.Red
                    Me.DueInfo.ForeColor = Color.White
                    Me.DueInfo.Text = Str1 & "" & Count & " . TOTAL AMOUNT " & DueAmt & "."
                    Me.DueInfo.Tag = Number
                    Me.DueInfo.AccessibleDescription = ID
                Else
                    Me.DueInfo.BackColor = Color.DeepSkyBlue
                    Me.DueInfo.ForeColor = Color.Black
                    Me.DueInfo.Text = "THERE IS NO DUE EXIST IN CARD SELLING."
                    Me.DueInfo.Tag = ""
                    Me.DueInfo.AccessibleDescription = ""
                End If
            End If
    ...
    Private Sub OFFICESERIALCELLNUMBERSToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles OFFICESERIALCELLNUMBERSToolStripMenuItem.Click
        ...
        For K As Decimal = X1 To X2
            ...
            RowX.Item("Date_Business") = CDate(WDate) 'date Input
            Me.AukDatabaseDataSet.New_Trans.Rows.Add(RowX)
        Next
    End Sub
...
DOWORKD:
    ...
    Private Sub FILTER_EFFECTSx(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles FILTERUNSENDONLYToolStripMenuItem.Click, FILTERSENDONLYToolStripMenuItem.Click, FILTERDUEONLYToolStripMenuItem.Click, FILTERCARDSONLYToolStripMenuItem.Click, REMOVEFILTERToolStripMenuItem1.Click, FILTERSTATUSNULLSONLYToolStripMenuItem.Click, FILTERONLYCURRENTCOMPANYDUEToolStripMenuItem.Click, FILTERDUEONLYToolStripMenuItem1.Click, FILTERCARDSDUEONLYToolStripMenuItem.Click, FILTERSTATUSNULLSONLYToolStripMenuItem1.Click, FILTERONLYDUEMOBILERECHARGEDUEToolStripMenuItem.Click, FILTERONLYCARDSDUEToolStripMenuItem.Click, ToolStripMenuItem28.Click, DueFilterSplitBut.ButtonClick
        ...
        End If
    End Sub
    Private Sub SpecificCardsMsg_Method(Optional ByVal StrToGet As String = "", Optional ByVal DueSh As Boolean = False, Optional ByVal StockShow As Boolean = False, Optional ByVal SoldShow As Boolean = False)
        Dim TCom As String
        ', BenefitStr, CardSellingStr, CardStrArray
        If StrToGet = "due" Then
            DueSh = True
        ElseIf StrToGet = "stock" Then
            StockShow = True
        ElseIf StrToGet = "sold" Then
            SoldShow = True
        End If
        ...
        For K As Integer = 0 To Me.AukDatabaseDataSet.Mob_Cards.Rows.Count - 1
            Try
                ...
                If Count > 0 Then
                    ...
                    If DueCount <> 0 Then CardsDueStr = CardsDueStr & vbCrLf & CardStr2 & " Count: " & DueCount & ", Due Price: " & Due

                    If Benefit <> 0 Then BStr = BStr & vbCrLf & CardStr & Benefit
                    If Price <> 0 Then PStr = PStr & vbCrLf & CardStr & Price
                    If Count <> 0 Then CountStr = CountStr & vbCrLf & CardStr & Count
                    If StockCount <> 0 Then StockStr = StockStr & vbCrLf & CardStr & StockCount
                    TPrice += Price
                    TBenefit += Benefit
                    TCount += Count
        ...
        If TCount > 0 Then
            ...
            If SoldShow = True Then MsgBox(PStr & vbCrLf & vbCrLf & "Total: " & TPrice, MsgBoxStyle.Information, "Individual Cards Selling Price(Without Due)")
            ...
            If StockShow = True Then MsgBox(StockStr & vbCrLf & vbCrLf & "Total: " & TStockCount, MsgBoxStyle.Information, "Individual Cards InStock Information")
            If DueSh = True Then MsgBox(CardsDueStr & vbCrLf & vbCrLf & "Total: " & TDue, MsgBoxStyle.Information, "Individual Cards Due Information")
        Else
...
lastend:    MsgBox("Information of Cards is not found in this Records.", MsgBoxStyle.Information)
    ...
    Private Sub LOADTODAYSDATABASEVIEWToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LOADTODAYSDATABASEVIEWToolStripMenuItem.Click, OpenTodayInforamtionToolStripMenuItem.Click
        'If Me.DateBusiness.Enabled = False Then
        If Me.AukDatabaseDataSet.HasChanges = True Then
            If SavMsg("DO YOU WANT TO LOAD NEW IF YOU WANT THEN PLEASE CHOOSE THE OPTION FOR SAVE ." & vbCrLf & "YES=SAVE AND LOAD FOR NEW DATE,NO=STOP SAVE AND DO FOR NEW DATE,CANCEL= CANCEL DOING ALL.") = 1 Then
                SAVE_MOBS()
    ...
    Private Sub YesterDayDataLoad(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripMenuItem30.Click, OpenYesterdayInformationToolStripMenuItem.Click
        'If Me.DateBusiness.Enabled = False Then
        If Me.AukDatabaseDataSet.HasChanges = True Then
            If SavMsg("DO YOU WANT TO LOAD NEW IF YOU WANT THEN PLEASE CHOOSE THE OPTION FOR SAVE ." & vbCrLf & "YES=SAVE AND LOAD FOR NEW DATE,NO=STOP SAVE AND DO FOR NEW DATE,CANCEL= CANCEL DOING ALL.") = 1 Then
                SAVE_MOBS()
        ...
        End If
        'End If
        BusEnb(CDate(NDate()).AddDays(-1))

    End Sub
    Private Sub DataErrorhandle(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewDataErrorEventArgs) Handles New_TransDataGridView.DataError
        GrdErrMsg(e)
    End Sub

    Private Sub Execute_Date_ButtonEffect(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles OpenOnlyDueByCellNumberToolStripMenuItem.Click, OpenOnlyDueByDateToolStripMenuItem.Click, OpenByDateToolStripMenuItem.Click, OpenByCellNumberToolStripMenuItem1.Click, OpenByCellNumberToolStripMenuItem.Click
        a = sender.tag
        If a = "dd" Then 'due by date
            If Me.AukDatabaseDataSet.HasChanges = True Then
                If AukF.MsgTr("DO YOU WANT TO SAVE THOSE EDITED INFORMATION?", False, MsgBoxStyle.Information) = 1 Then
                    SAVE_MOBS()
                    GoTo DOWORKD
...
DOWORKD:
            Try
                If CSOFT.Date_Get_FromStr(DateBusiness.Text) = True Then
                    ax = WGeT
                    SFC("Date_Business", "STATUS")
                    STC(CDate(ax), "SEND BUT NOT PAID YET")
                    DTC("d")
                    SnX("=")
                    AukF.Db_LoadN(Me.AukDatabaseDataSet.New_Trans, -1, "", True)
                    SFC("Date_Business", "dUE_cARDS")
            ...
            Catch ex As Exception
                Epx()
            End Try
        ElseIf a = "dc" Then 'due by cell or id
            Me.DueCheck(DateBusiness.Text)
        ElseIf a = "oc" Then 'open by cell  number
            LOAD_CLIENTS_INFOR()
        ElseIf a = "od" Then 'open by date
            BusEnb()
        ElseIf a = "c" Then 'open by cell number
            LOAD_CLIENTS_INFOR()

        End If
    End Sub

    Private Sub Show_Cards_indi_info2(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripMenuItem29.Click, STOCKINFORMATIONToolStripMenuItem.Click, SOLDINFORMATIONToolStripMenuItem.Click
        Me.SpecificCardsMsg_Method(sender.tag)
    End Sub

    Private Sub ToolStripTextBox10_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles ToolStripTextBox10.KeyDown
        If e.KeyCode = Keys.Enter Then
            Dim D1 As Date
            If IsDate(ToolStripTextBox9.Text) = True Then
                Try
                    D1 = CDate(ToolStripTextBox9.Text)
                Catch ex As Exception
                    Epx()
                    Exit Sub
                End Try
            Else
                ...
                Try
                    D1 = CSOFT.NumToDate(Val(ToolStripTextBox9.Text))
                Catch ex As Exception
                    Epx()
                    Exit Sub
                End Try
            ...
            SJOIN = "OR"
            BracketsSql = False
            Str1 = AukSql.AukSql_Main(-1, Me.AukDatabaseDataSet.New_Trans.TableName.ToString)
            Str2 = SoftIn.GetFunction_QrText(Me.AukDatabaseDataSet.New_Trans, "Date_business", D1, 0)
            sm = "(" & str1 & ") AND (" & str2 & ")"
            AukF.Db_LoadQ(Me.AukDatabaseDataSet.New_Trans, sm, -1, False)
            Me.New_TransBindingSource.Filter = BSql
            'MsgBox(BSql)
        End If

    End Sub

    Private Sub New_TransDataGridView_UserAddedRow(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewRowEventArgs) Handles New_TransDataGridView.UserAddedRow
        Master_ID_String(Drv, 11)
    End Sub
    Public Sub SaveAuto()
        Me.SAVE_MOBS()
    End Sub
    Private Sub TillDueLoad(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LOADTILLDUEToolStripMenuItem.Click
        Try
            If CSOFT.CheckFormDBChg_GoOn(Me.AukDatabaseDataSet, Me, False) = False Then Exit Sub
            SFC("Status", "Due_Cards")
            STC(DueStr, "True")
            DTC("", "b")
            SJOIN = "or"
            AukF.Db_LoadN(Me.New_TransBindingSource)
        Catch ex As Exception
            Epx()
        End Try
    ...
    Private Sub DATEINFOR_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DATEINFOR.Click
        ToolStripTextBox8.Text = ""
        ToolStripTextBox8.Focus()
    End Sub
    Private Sub DueSHowByDoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DueInfo.DoubleClick
        'Dim ID As Boolean = AukF.CheckVal_GetBool(Me.DueInfo.AccessibleDescription.ToString)
        'Dim STr1 As String
        'If ID = True Then
        If Me.DueInfo.Tag <> "" Then
            Me.DueCheck(Me.DueInfo.Tag)
            Me.DueInfo.BackColor = Color.DeepSkyBlue
            Me.DueInfo.ForeColor = Color.Black
            'Me.DueInfo.Text = "THERE IS NO DUE EXIST IN CARD SELLING."
            Me.DueInfo.Tag = ""
            Me.DueInfo.AccessibleDescription = Me.DueInfo.Tag
        Else
            Me.LOADTODAYSDATABASEVIEWToolStripMenuItem_Click(sender, e)
            Me.DueInfo.Tag = Me.DueInfo.AccessibleDescription
            Me.DueInfo.BackColor = Color.Red
            Me.DueInfo.ForeColor = Color.White
        End If
    ...
    Private Sub DueInfo_MouseHover(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DueInfo.MouseHover
        Me.ToolTip1.SetToolTip(sender, "DOUBLE CLICK TO SHOW DUE AND PRESS" & vbCrLf & "CTRL+L OR IF YOU DOUBLE CLICK ON THIS YOU CAN GO BACK TO YOUR TODAY'S WORK.")
    End Sub

    Private Sub LaodVideo(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TelecomCompanyAddToolStripMenuItem.Click, SummaryToolStripMenuItem.Click, SpecialFeaturesToolStripMenuItem.Click, SettingsToolStripMenuItem.Click, MemberCreateToolStripMenuItem.Click, HowToSearchToolStripMenuItem.Click, ErrorSolutionToolStripMenuItem.Click, DueCheckToolStripMenuItem.Click, CardsSellingToolStripMenuItem.Click, BasicsToolStripMenuItem.Click
        Dim Path As String = AppPath & "\VHELp\MRECharge\" & sender.text & ".avi"
        If My.Computer.FileSystem.FileExists(Path) = True Then
            AukF.OExe(Path, Me)
        Else
            MsgBox("Help File is not Buil-in.", MsgBoxStyle.Exclamation)
        End If
    End Sub
    ...
    Private Sub ToolStripMenuItem31_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripMenuItem31.Click
        Dim Path As String = AppPath & "\VsInstall Codec.exe"
        If My.Computer.FileSystem.FileExists(Path) = True Then
            AukF.OExe(Path, Me)
        Else
            MsgBox("Codec File is not Built-in.", MsgBoxStyle.Exclamation)
        End If
    End Sub

    Private Sub SIMSELLINGToolStripMenuItem1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SIMSELLINGToolStripMenuItem1.Click
        FrmL(SimSold)
    End Sub
End Class