Public Class Product_Input3
    ...
    Dim DVDIDAdp As New AukDatabaseDataSetTableAdapters.DVDIDSTableAdapter
    Dim DvDRDelAdp As New AukDatabaseDataSetTableAdapters.DVD_RentDeletedTableAdapter
    ...
    Private Sub Product_Input3_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Activated
        ...
        End If
    End Sub
    Public Sub RefreshDVDIDS()
        Me.DVDIDAdp.Fill(Me.AukDatabaseDataSet.DVDIDS)
    End Sub
    ...
    Private Sub Product_input_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        'Try
        '    SQL1 = "DELETE  * FROM [DVD_RENTDELETED] WHERE [DELETED_DATE]< #" & Dat2(NDate, False).AddDays(-Val(My.Settings.DVOpt_DelRowsKeep)) & "#"
        '    MsgBox(SQL1)
        '    Adp = New OleDb.OleDbDataAdapter
        '    Adp.DeleteCommand.Connection = Cn
        '    Adp.DeleteCommand.CommandText = SQL1
        '    Adp.DeleteCommand.CommandType = CommandType.Text
        'Catch ex As Exception

        'End Try
        If My.Settings.Dont_DeleteDVDRentDeleted = False Then
            If Val(My.Settings.DVOpt_DelRowsKeep) > -1 Then
                ...
                Next
                Me.DvDRDelAdp.Update(Me.AukDatabaseDataSet.DVD_RentDeleted)
            End If
        End If
    

        'ViewQuery.DataGrid1.DataSource = Me.AukDatabaseDataSet.DVD_RentDeleted
        'FrmL(ViewQuery)
        'Adp..ExecuteReader()
        ...
        DVDIDAdp.Fill(Me.AukDatabaseDataSet.DVDIDS)
    ...
    Private Sub Product_CollectionDataGridView_CellBeginEdit(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellCancelEventArgs) Handles Product_CollectionDataGridView.CellBeginEdit
        If e.ColumnIndex = 0 Then PreviousId = AukF.GridT(Me.Product_CollectionDataGridView, 0, e.RowIndex)
    ...
    Private Sub Product_CollectionDataGridView_CellEndEdit(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles Product_CollectionDataGridView.CellEndEdit
        ...
        If My.Settings.DVDOPT_AUTOMACI_DVDID = True Then
            ...
            If D(0, e.RowIndex).Value.ToString = "" Then 'if dvd is is null
                If e.RowIndex = 0 Then 'if index=0
                    'SoftIn.DataGrid_MasterID_ByDate(D, 10)
                    If ADSRC.FIND_FROM_DATATABLE_INDEX(Me.AukDatabaseDataSet.DVDIDS, "Num") = True Then
                        AukF.GridTSet(sender, Val(RowX(1).ToString) + 1, 0, -1, True)  'insert max dvdid value
                    Else
                        AukF.GridTSet(sender, 1, 0, -1, True)  'insert max dvdid value
                    End If
                Else
                    'movie id eneting
                    If D(0, e.RowIndex - 1).Value.ToString <> "" Then 'if before row id isNot valid
                        If IsNumeric(D(0, e.RowIndex - 1).Value.ToString) = True Then 'if num then enter auto num
                            Dim BeNum As Decimal
                            Dim FStr, NumStr As String
                            NumStr = CDec(D(0, e.RowIndex - 1).Value.ToString) + 1
                            If ADSRC.FIND_FROM_DATATABLE_INDEX(Me.AukDatabaseDataSet.DVDIDS, "Num") = True Then 'for numbers.ID=Num
                                'if Num key is created on dvdids db and found
                                If Val(RowX(1).ToString) < Val(NumStr) Then
                                    RowX(1) = Val(NumStr)
                                    DVDIDAdp.Update(RowX)
                                    D(0, e.RowIndex).Value = NumStr
                                ElseIf Val(RowX(1).ToString) >= Val(NumStr) Then 'if before num is less than maximan save then inout maximam
                                    MaxDvdID = Val(RowX(1).ToString) + 1
                                    RowX(1) = CDec(MaxDvdID)
                                    DVDIDAdp.Update(RowX)
                                    D(0, e.RowIndex).Value = MaxDvdID
                                    'D(0, e.RowIndex).Value = FStr & Val(RowX(1).ToString) + 1

                                    'insert max dvdid value
                                End If
                            Else
                                'if num key is not found in dvdid db
                                RowX = Me.AukDatabaseDataSet.DVDIDS.NewRow
                                RowX(0) = "Num"
                                RowX(1) = Val(NumStr)
                                D(0, e.RowIndex).Value = NumStr
                                Me.AukDatabaseDataSet.DVDIDS.Rows.Add(RowX)
                                DVDIDAdp.Update(RowX)
                            End If
                        ...
                        Else
                            ...
                            NA1 = NB1.Split(My.Settings.DVDID_TOKEN.ToString).GetValue(1).ToString 'get the last value after tokne, such as tx-100 , get 100
                            NumStr = Val(na1) + 1 'number of before id
                            FStr = NB1.Split(My.Settings.DVDID_TOKEN.ToString).GetValue(0).ToString & My.Settings.DVDID_TOKEN.ToString 'token before value ,as eg  tx and add token (-), so result tx-
                            If ADSRC.FIND_FROM_DATATABLE_INDEX(Me.AukDatabaseDataSet.DVDIDS, FStr) = True Then
                                If Val(RowX(1).ToString) < Val(NumStr) Then
                                    RowX(1) = Val(NumStr)
                                    DVDIDAdp.Update(RowX)
                                    D(0, e.RowIndex).Value = FStr & NumStr
                                ElseIf Val(RowX(1).ToString) >= Val(NumStr) Then 'if before num is less than maximan save then inout maximam
                                    'database id is greater n new value is less than that, we have to put last databse saved id number +1 and
                                    ' if equal we have to do this same
                                    MaxDvdID = Val(RowX(1).ToString) + 1
                                    RowX(1) = Val(MaxDvdID)
                                    DVDIDAdp.Update(RowX)
                                    D(0, e.RowIndex).Value = FStr & MaxDvdID
                                End If
                            Else
                                'if dvd new id token is not exist in databse then inset and save last value
                                RowX = Me.AukDatabaseDataSet.DVDIDS.NewRow
                                RowX(0) = FStr
                                RowX(1) = Val(NumStr)
                                D(0, e.RowIndex).Value = NumStr
            ...
            End If
        End If
        If e.ColumnIndex = 0 Then 'if edit on dvd id row
            If PreviousId = AukF.GridT(sender, 0, e.RowIndex) Then GoTo nextstep 'because .. id is same as before and before id is valided before so no need to valid and insert new id

            'this block is for slove the problem on personal edit on dvdid field
            s = (AukF.GridT(sender, 0, -1, 0).ToString)
            If IsNumeric(s) = False Then
                If s <> "" AndAlso InStr(s, My.Settings.DVDID_TOKEN.ToString) > 0 Then
                    Dim FStr, NumStr As String
                    NB1 = s
                    NA1 = NB1.Split(My.Settings.DVDID_TOKEN.ToString).GetValue(1).ToString
                    NumStr = Val(na1)  'number of before id
                    FStr = NB1.Split(My.Settings.DVDID_TOKEN.ToString).GetValue(0).ToString & My.Settings.DVDID_TOKEN.ToString
                    If ADSRC.FIND_FROM_DATATABLE_INDEX(Me.AukDatabaseDataSet.DVDIDS, FStr) = True Then
                        If Val(RowX(1).ToString) < Val(NumStr) Then 'if insert number is greater than db saved value then input this new value to databse and save
                            RowX(1) = Val(NumStr)
                            DVDIDAdp.Update(RowX)
                            D(0, e.RowIndex).Value = FStr & NumStr
                        ElseIf Val(RowX(1).ToString) >= Val(NumStr) Then 'if before num is less than maximan save then inout maximam
                ...
                End If
            Else 'if is numeric
                If IsNumeric(s) = True Then 'if num then enter auto num
                    Dim BeNum As Decimal
...
ba:
        Try
            If InStr(qw, ",") = 0 Then
                Dim Ntx As String
                If IsNumeric(qw) = True Then
                    Dim Max As Integer = ADSRC.MAXIMUM_VALUE("Product_Collection", "dvd_id")
                    For K As Integer = (Max + 1) To (Max + Val(qw))
                        If Ntx = "" Then
                            Ntx = K
                        Else
                            Ntx &= "," & K
                    ...
                    AukF.CommonRowsAdd(Rowq, Ntx, 0, "", "", ",", Me.Product_CollectionDataGridView, Me.ToolStripProgressBar1, True)
            ...
            Else
dones:          AukF.CommonRowsAdd(Rowq, qw, 0, "", "", ",", Me.Product_CollectionDataGridView, Me.ToolStripProgressBar1, True)
            End If
            Product_CollectionBindingNavigatorSaveItem_Click(sender, e)
        Catch ex As Exception
            Epx()
...
ADDIN:              CUS_ID = CStr(CusR.Costomer_ID).ToString.ToUpper
    ...
    End Function
    Public Function ReturnDate_byMaxProgrammage(ByVal R2 As AukDatabaseDataSet.Product_CollectionRow) As Date
        InsDate = CDate(R2.Item("Inserted_date").ToString)
        Distance = DateDiff(DateInterval.Day, InsDate, CDate(NDate()))
        If Distance > Val(My.Settings.InsMNewDays_DvdOpt.ToString) Then
            'old movie ena,old maximum rent in days ,=rent_Days
    ...
    Public Sub SaveP2()
        ...
        Catch ex As Exception
            Epx()
        End Try
        If Me.DVD_RentBindingSource.Count <= 200 Then
            AukF.Mother_DB_Load(Me.DVD_RentBindingSource, 4)
            SFC("Dvd_id")
            STC(GetStrs)
            ExpreC("i")
            AukF.Db_Load("DVD_ID,sum(pay_money),Sum(Fine)", "Dvd_rent", MTb, True, "DVD_ID", False, False, 200, 0, True)
            For Aq = 0 To MTb.Rows.Count - 1
                ...
                If AukF.BindFind(Me.DVD_RentBindingSource, "DVD_ID", DvdId) = True Then
                    ...
                    Fine = Me.AukDatabaseDataSet.DVD_Rent(ComRow).Item("fine").ToString
                End If
                ...
                If Row IsNot Nothing Then
                    ...
                    Row("place") = Place
                    If Place <> "InStore" AndAlso Place <> "InHand" Then
                        Row("Sold_or_Broken_Price") = Fine
                        Row("Discount") = 0
                    End If
    ...
    Private Sub SavePro(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SaveProgramaticToolStripMenuItem.Click, ToolStripButton7.Click, SAVEPROGRAMMATICWAYToolStripMenuItem.Click, ToolStripMenuItem24.Click, ToolStripMenuItem58.Click, ToolStripMenuItem75.Click
        ...
        Else
            ...
            End If
        End If
        Me.due_CheckAddNewMov.Checked = False
        SaveCom()
    End Sub
    Public Sub REnd(ByVal Aw As Integer)
        ...
        End If
    End Sub
    Private Sub EXCHANGEMOVIE(ByVal NEWDVDID As String, ByVal Pos As Integer)
        'If AukF.GridT(Me.DVD_RentDataGridView, 6, Pos) <> "InStore" AndAlso AukF.GridT(Me.DVD_RentDataGridView, 6, Pos) <> "InHand" AndAlso AukF.GridT(Me.DVD_RentDataGridView, 6, Pos) <> "" Then
        If Rent = False Then MsgBox("Can't Exchange Movie When No Customer is Loaded.", MsgBoxStyle.Exclamation)
        Dim StockR As AukDatabaseDataSet.Product_CollectionRow
        Dim RentR As AukDatabaseDataSet.DVD_RentRow
        Dim RentID As String = AukF.GridT(Me.DVD_RentDataGridView, 18, Pos) 'id of rent
        Dim DVDID As String

        If RentID <> "" Then
            RentR = Me.AukDatabaseDataSet.DVD_Rent.FindByID(RentID)
            If RentR Is Nothing Then MsgBox("Current Row Couldn't Find") : Exit Sub
            DVDID = RentR.DVD_ID.ToString
            Dim RPlace As String = RentR.Item("Place").ToString
            Dim RPal As String = LCase(RPlace)
            If ADSRC.FIND_FROM_DATATABLE_INDEX(Me.AukDatabaseDataSet.Product_Collection, DVDID, 6) = True Then 'place

Wkr:            StockR = RowX 'prodcut col row
                StockR.Place = "InStore"
                Me.Product_CollectionTableAdapter.Update(StockR)

                If ADSRC.FIND_FROM_DATATABLE_INDEX(Me.AukDatabaseDataSet.Product_Collection, NEWDVDID) = True Then
                    'NewMovRow 
newadd:             StockR = RowX
                    If StockR.Place = "InStore" Then
                        RentR.DVD_ID = StockR.DVD_ID.ToString
                        RentR.Movie_Name = StockR.Item("Movie_Name").ToString
                        StockR.Costomer_ID = RentR.Costomer_ID
                        RentR.Date_Rent = Now
                        RentR.Pay_Money = Val(StockR.Item("Rent_Price").ToString)
                        RentR.Return_date = ReturnDate_byMaxProgrammage(StockR)
                        StockR.Place = "InHand"
                        'we don;t save because if its a new added movie then its rent cost wil no add
                        Me.Product_CollectionTableAdapter.Update(StockR)
                        'Me.DVD_RentTableAdapter.Update(RentR)
                    Else
                        MsgBox("Your can't Exchange the Movies which place is already 'InHand'", MsgBoxStyle.Exclamation)

                    End If
                
                Else
                    If DvDloadByID(NEWDVDID) = True Then GoTo newadd Else MsgBox("Current New Movie Is Not Found in Database.But Before Movie's place set to Instore.") 'its returns rowx
                End If

            Else 'not found in db
                If DvDloadByID(DVDID) = True Then GoTo Wkr Else MsgBox("Before Movie is not Found in database.", MsgBoxStyle.Critical) 'its returns rowx
            End If
    ...
    End Sub
    Public Function DvDloadByID(ByVal ID As String) As Boolean
        SFC("DVD_ID")
        STC(ID)
        AukF.Db_LoadN(Me.AukDatabaseDataSet.Product_Collection, -1, "", False)
        DvDloadByID = ADSRC.FIND_FROM_DATATABLE_INDEX(Me.AukDatabaseDataSet.Product_Collection, ID)
    End Function

    Private Sub SendRentPlaceToStock(ByVal Pos As Integer)
        Try
            If AukF.GridT(Me.DVD_RentDataGridView, 6, Pos) <> "InStore" AndAlso AukF.GridT(Me.DVD_RentDataGridView, 6, Pos) <> "InHand" AndAlso AukF.GridT(Me.DVD_RentDataGridView, 6, Pos) <> "" Then
                Dim StockR As AukDatabaseDataSet.Product_CollectionRow
                Dim RentR As AukDatabaseDataSet.DVD_RentRow
                ...
                If RentID <> "" Then
                    RentR = Me.AukDatabaseDataSet.DVD_Rent.FindByID(RentID)
                    If RentR Is Nothing Then MsgBox("Current Row Couldn't Find") : Exit Sub
                    Dim DVDID As String = RentR.DVD_ID.ToString
                    Dim RPlace As String = RentR.Item("Place").ToString
                    Dim RPal As String = LCase(RPlace)
                    If ADSRC.FIND_FROM_DATATABLE_INDEX(Me.AukDatabaseDataSet.Product_Collection, DVDID, 6) = True Then 'place

Wkr:                    StockR = RowX 'prodcut col row
                        Dim Fine As Decimal = Val(RentR.Item("Fine").ToString)
                        If LCase(WGeT) <> LCase(RPlace) Then
                            If RentR.Due = True Then
                                If MsgBox("Current Customer Borrow This Movie in Due Status.Would You Like to Make Him/Her Due=False.", MsgBoxStyle.Exclamation + MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then RentR.Due = False Else MsgBox("Please Make Current Costomer Due=False ,Or Else Current Record will be Show as Error Record.", MsgBoxStyle.Exclamation)
                            End If
                            StockR.Place = RPlace
                            If Fine = 0 Then
                                Fine = Val(InputBox("Please Input a Number Value for Get the Movies Fine Cost,you can type your movies Sold Cost.", "Movies Fine", Fine))
                                StockR.Sold_or_Broken_Price = Fine
                                StockR.Discount = 0
                                RentR.Fine = Fine
                                MsgBox("Current Movie is Sold to Current Client(" & StockR.Item("Costomer_ID").ToString & ") By " & Fine & " T.K.", MsgBoxStyle.Information)
                    ...
                    Else 'dvd id isnot found in db,then load and goto again to work
                        SFC("DVD_ID")
                        STC(DVDID)
                        AukF.Db_LoadN(Me.AukDatabaseDataSet.Product_Collection, -1, "", False)
                        If ADSRC.FIND_FROM_DATATABLE_INDEX(Me.AukDatabaseDataSet.Product_Collection, DVDID, 6) = True Then 'place
    ...
    Private Sub OpenExactFromFirstToolStripMenuItem1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles OpenExactFromFirstToolStripMenuItem1.Click
        ...
        AukF.ComSelIndex(ToolStripComboBox5)
        SFC(ToolStripComboBox3.Text, "Place")
        STC(ToolStripTextBox4.Text, ToolStripComboBox5.Text)
        AukF.Db_Load("*", "Dvd_rent", Me.DVD_RentBindingSource, True, "", True, True, Val(My.Settings.MaximumData_load), -1)
        'If Me.DVD_RentBindingSource.DataSource Is Nothing Then
        '    Me.DVD_RentBindingSource.DataSource = Me.AukDatabaseDataSet.DVD_Rent
        'End If
    ...
    Private Sub OpenToolStripMenuItem3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles OpenToolStripMenuItem3.Click
        ...
        'MsgBox(Sql)
        If Me.DVD_RentDataGridView.DataSource Is Nothing Then
            Me.DVD_RentDataGridView.DataSource = Me.AukDatabaseDataSet.DVD_Rent
        End If
    End Sub
    ...
    End Sub
    Private Sub AdvanceSearchQueryToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles AdvanceSearchQueryToolStripMenuItem.Click, ToolStripMenuItem50.Click
        FrmL(AdvanceSearch)
    End Sub
    ...
    Private Sub Delbuts_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Delbuts.Click, DELETECURRENTDATAOBJECTToolStripMenuItem.Click
        ...
        End If
        If Me.DVD_RentDataGridView.CurrentCell Is Nothing Then Exit Sub
        'DVDiD = Me.AukDatabaseDataSet.DVD_Rent(Me.DVD_RentBindingSource.Position).DVD_ID.ToUpper
        'get frm grid
        DVDiD = AukF.GridT(Me.DVD_RentDataGridView, 1).ToString 'dvdid
        Dim RentID As String = AukF.GridT(Me.DVD_RentDataGridView, 18).ToString 'Rent DBID
        If Me.DVD_RentDataGridView.CurrentCell.RowIndex > -1 Then
            If Val(My.Settings.DVOpt_DelRowsKeep) > -1 AndAlso ADSRC.FIND_FROM_DATATABLE_INDEX(Me.AukDatabaseDataSet.DVD_Rent, RentID) = True Then
                DelRow = RowX
                Dim IT As Array
                IT = RowX.ItemArray
                Dim Km(22) As Object
                For L As Integer = 0 To IT.Length - 1
                    Km(L) = ArV2(IT, L, DBNull.Value)
                Next
                'IT.Resize(IT, IT.Length + 3)
                'Array.Resize(IT , IT.Length + 3)
                Km(20) = NDate()
                Km(21) = CStr(TimeOfDay)
                'it(23)=
                Try
                    Me.AukDatabaseDataSet.DVD_RentDeleted.Rows.Add(Km)
                    Me.DvDRDelAdp.Update(Me.AukDatabaseDataSet.DVD_RentDeleted)
                Catch ex As Exception
                    'Epx()
                    Try
                        Km(0) = SoftIn.Master_ID_String_Get(6, 0)
                        Me.AukDatabaseDataSet.DVD_RentDeleted.Rows.Add(Km)
                        Me.DvDRDelAdp.Update(Me.AukDatabaseDataSet.DVD_RentDeleted)
            ...
            End If
            'If Me.DVD_RentBindingSource.Position > -1 Then
            '    Me.AukDatabaseDataSet.DVD_Rent(Me.DVD_RentBindingSource.Position).Delete()
            'End If
            DelRow.Delete()
            Me.DVD_RentTableAdapter.Update(DelRow)
    ...
    Private Sub SAVE_DVDRENT(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DVD_RentDataGridView.CellEndEdit
        If e.ColumnIndex = 11 Then
            Me.TRntC()
        End If
        Dim Place As String = AukF.GridT(sender, 6)
        If e.RowIndex > -1 Then
            If Place = "InStore" Then
                'fine=-1 or recevid date=""
                recxa = AukF.GridT(sender, 12, -1, "") 'recevded date
                If (Val(AukF.GridT(sender, 11, -1, -1)) = -1 OrElse Val(AukF.GridT(sender, 11, -1, -1)) = 0) AndAlso recxa = "" Then '11=fine
                    REnd(e.RowIndex)
                End If
            ElseIf Place <> "InStore" OrElse Place <> "InHand" Then
                If e.RowIndex > -1 Then
                    Me.SendRentPlaceToStock(e.RowIndex)
    ...
    Private Sub GETMAXIMAMDVDIDOFSTRINGToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles GETMAXIMAMDVDIDOFSTRINGToolStripMenuItem.Click
        Dim TX1 As New DataTable("DVDIDS")
        ix = InputBox("Please Input the Id First Chrs To Get Maximam ID Value of String.", "DVD ID String", ix)
        If ix <> "" Then
            'SFC("id")
            'STC(ix)
            'ADSRC.Load_Only_LastRecord_Str(TX1, 0, "DVD_ID", True)
            'C = Val(Replace(WGeT, ix, "", 1, -1, CompareMethod.Text))
            If ADSRC.FIND_FROM_DATATABLE_INDEX(Me.AukDatabaseDataSet.DVDIDS, ix, 1) = True Then
                If AukF.GridT(Me.Product_CollectionDataGridView, 0) = "" Then
                    AukF.GridTSet(Me.Product_CollectionDataGridView, ix & Val(RowX.Item(1).ToString) + 1, 0, -1, True)
                Else
                    MsgBox(ix & Val(RowX.Item(1).ToString))
                End If
            Else
                MsgBox("Your Typed ID isnot Created Before.", MsgBoxStyle.Exclamation)
            End If
        End If

    End Sub

    Private Sub DVDSTRINGIDDATABASEToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DVDSTRINGIDDATABASEToolStripMenuItem.Click
        FrmL(DVDIDSFrm)
    End Sub

    Private Sub ErrorSolve(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripMenuItem80.Click
        'For k As Integer = 0 To Me.Product_CollectionDataGridView.SelectedRows.Count - 1
        On Error GoTo b
        Dim Str1 As String
        Dim T As DataTable
        Str1 = ADSRC.ROWC_Get_In_InCommand_Grd(Me.Product_CollectionDataGridView, 0, 0, False) 'Only get names a,b,c as which place is "inHand" 
        SFC("a")
        STC(Str1)
        ExpreC("q")
        T = ADSRC.AdvSummSys_RTable(Me.AukDatabaseDataSet.DVDMainError, "DVD_ID,DVD_Rent.Costomer_ID,Rent Place", "", "", False)

        If T.Rows.Count > 0 Then
            ToolStripProgressBar1.Visible = True
            ToolStripProgressBar1.Maximum = T.Rows.Count + 1
            ToolStripProgressBar1.Value = 0
            For K As Integer = 0 To T.Rows.Count - 1
                Dim DID As String = T.Rows(K).Item(0).ToString
                Dim Customer As String = T.Rows(K).Item(1).ToString
                Dim Placexq As String = T.Rows(K).Item(2).ToString

                If ADSRC.FIND_FROM_DATATABLE_INDEX(Me.AukDatabaseDataSet.Product_Collection, DID) = True Then
                    RowX.Item("Place") = Placexq
                    RowX.Item("Costomer_ID") = Customer
                    Me.Product_CollectionTableAdapter.Update(RowX)
                End If
        ...
        End If
        Exit Sub
b:      Epx() : ToolStripProgressBar1.Visible = False : Exit Sub

        'Next
    End Sub

    Private Sub ErrorDatabase(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripMenuItem81.Click
        Try
            AukF.Db_LoadN(Me.AukDatabaseDataSet.DVDMainError)
            ViewQuery.DataGrid1.DataSource = Me.AukDatabaseDataSet.DVDMainError
            FrmL(ViewQuery)
        Catch ex As Exception
            Epx()
    ...
    Private Sub SHOWERRORRECORDSToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SHOWERRORRECORDSToolStripMenuItem.Click
        ...
        SJOIN = "AND"
        BracketsSql = False
        s1 = AukSql.AukSql_Main(-1, "DVD_RENT")
        ...
        'SnX("<>")
        BracketsSql = False
        s2 = AukSql.AukSql_Main(-1, "DVD_RENT")
        s = "(" & s1 & ") AND (" & s2 & ")"
        'MsgBox(s)
        AukF.Db_LoadQ(Me.AukDatabaseDataSet.DVD_Rent, s)
        's1 = AukSql.AukSql_Main(-1, "DVD_RENT")
        'SFC("Place", "Fine")
        'STC("InStore", 0)
        'SnX("<>")
    ...
    Private Sub DVDRENTADMINOPTIONSToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DVDRENTADMINOPTIONSToolStripMenuItem.Click
        FrmL(DVDRENT_ADMIN)
    End Sub

    Private Sub eXCHANGEMOVIE(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles ToolStripTextBox14.KeyDown
        If e.KeyCode = Keys.Enter Then
            If Me.DVD_RentDataGridView.CurrentCell IsNot Nothing Then EXCHANGEMOVIE(sender.text, Me.DVD_RentDataGridView.CurrentCell.RowIndex)
        End If
    End Sub

    Private Sub RESOLVETHEERRORSAUTOMATICToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RESOLVETHEERRORSAUTOMATICToolStripMenuItem.Click
        'For k As Integer = 0 To Me.Product_CollectionDataGridView.SelectedRows.Count - 1
        On Error GoTo b
        Dim Str1 As String
        Dim T As DataTable
