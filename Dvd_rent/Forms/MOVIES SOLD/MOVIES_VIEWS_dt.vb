Public Class MOVIES_VIEWS
    ...
    Private Sub PS_VIEW_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Me.DateTimePicker2.Value = Dat(Now.Date).AddDays(7)
    ...
    Private Sub TextBox1_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles TextBox1.KeyDown
        'ADSRC.TWICE_EXE(sender, e, Me.PSSOLDVBindingSource, "sold id", "MOVIE ID", True, False, 0, 0)
        If QEnterTo_Query_FObject_and_Select_DataGridObject(sender, e, Me.AukDatabaseDataSet.MOVIES_SOLD_V, dGRID, "SOLD ID,MOVIE ID,MOVIE NAME", 0, "MOVIE NAME", "1", False, -1, Val(My.Settings.MaximumData_load)) = True Then
            Me.PSSOLDVBindingSource.Filter = BSql
            RPIDX = Me.PS_SOLDDataGrid(Me.PS_SOLDDataGrid.CurrentRowIndex, 11).ToString
            If RPIDX <> "" Then
                ADSRC.LOAD_AND_FIND(Me.AukDatabaseDataSet.MOVIES_RPLIST, "REPORT ID", RPIDX, -1, False, -1)
                Me.PS_VIEW1BindingSource.Filter = BSql
            End If
        ElseIf ADSRC.QEnterTo_Query_FObject_and_Select_DataGridObject(sender, e, Me.AukDatabaseDataSet.MOVIES_RPLIST, dGRID, "REPORT ID,CLIENT NAME,SOLD DATE,PREPARED BY", 0, Nothing, Nothing, False, -1, Val(My.Settings.MaximumData_load)) = True Then
            Me.PS_VIEW1BindingSource.Filter = BSql
            If My.Settings.VIEW_DETAILS_PS = False Then Exit Sub
            RPIDX = Me.PS_VIEW1DataGrid(Me.PS_VIEW1DataGrid.CurrentRowIndex, 0).ToString
            If RPIDX <> "" Then
                ADSRC.LOAD_AND_FIND(Me.AukDatabaseDataSet.MOVIES_SOLD_V, "REPORT ID", RPIDX, -1, False, -1)
                Me.PSSOLDVBindingSource.Filter = BSql
            End If
        End If
    End Sub
    ...
    Private Sub TextBox1_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TextBox1.TextChanged
        If ADSRC.TWICE_BINDFIND(Me.PSSOLDVBindingSource, "sold id", "MOVIE ID", sender.text) Then
        ...
        End If
    End Sub
    Public Function Q_IN_DetailsView(ByVal Tab As String, ByVal QTxt As String, Optional ByVal Report_only As Boolean = False)
        SFC(Tab)
        STC(QTxt)
        AukF.Db_LoadN(Me.AukDatabaseDataSet.MOVIES_SOLD_V, -1, "", False, 0, "", False, -1, Val(My.Settings.MaximumData_load))
        If Me.PS_SOLDDataGrid.VisibleRowCount > 0 Then
            Try
                rp = Me.PS_SOLDDataGrid(Me.PS_SOLDDataGrid.CurrentRowIndex, 10).ToString
            Catch ex As Exception
                rp = ""
            End Try
            SFC("report id")
            STC(rp)
            AukF.Db_LoadN(Me.AukDatabaseDataSet.MOVIES_RPLIST, -1, "", False, 0, "", False, 0, 1)
            If Report_only = False Then FrmL(Me) Else PrINT()
        End If
    End Function
    Public Function Load_date(ByVal D1 As String, ByVal d2 As String)
        Dim Da, Da2 As Date
        Try
            Da = Dat(D1) : Da2 = Dat(d2)
        Catch ex As Exception
            Epx()
            Exit Function
        End Try
        ...
        If Me.SHOWDETAILSOFSOLDPRODUCTSToolStripMenuItem.Checked = True Then
            AukF.BindFilter_BetweenOparetorN(Me.PSSOLDVBindingSource, "Sold date", Da, Da2, False, Me.WITHFILTERToolStripMenuItem.Checked, True, True)
        End If
        FILTERS()
        'MsgBox(Sql)
    End Function
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Me.Load_date(Dat(Now.Date), Dat(Now.Date))
    End Sub
    ...
    Public Function DAYS_BENEFIT(ByVal DAYS As Decimal, Optional ByVal SIGN As String = "=")
        ...
        DTC("N")
        SnX(SIGN)
        AukF.Db_LoadN(Me.AukDatabaseDataSet.MOVIES_RPLIST)
        If Me.SHOWDETAILSOFSOLDPRODUCTSToolStripMenuItem.Checked = True Then
            SFC("SOLD TO NOW DURATION(DAY)")
            STC(DAYS)
            DTC("N")
            SnX(SIGN)
            AukF.Db_LoadN(Me.AukDatabaseDataSet.MOVIES_SOLD_V)
        End If
        FILTERS()
    End Function
    ...
    Private Sub Button5_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button5.Click
        Me.Load_date(Dat(Me.DateTimePicker1.Value.Date), Dat(Me.DateTimePicker2.Value.Date))
    End Sub
    ...
    End Sub
    Public Sub PrINT()
        Dim M As New MOVIES_BENEFIT_REPORT
    ...
    Private Sub SHOWDETAILSOFSOLDPRODUCTSToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SHOWDETAILSOFSOLDPRODUCTSToolStripMenuItem.Click
        My.Settings.Save()
    End Sub
    Private Sub REMOVEFILTERToolStripMenuItem_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles REMOVEFILTERToolStripMenuItem.Click
    ...
    Private Sub PS_SOLDDataGrid_DoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles PS_SOLDDataGrid.DoubleClick
        rpid = sender(Me.PS_SOLDDataGrid.CurrentRowIndex, 11)
    ...
    Private Sub SHOWAUKQUERYMANAGERToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Dim gD As DataGrid
        If My.Settings.VIEW_DETAILS_PS = True Then
            If dGRID.Name = Me.PS_SOLDDataGrid.Name Then
                gD = Me.PS_VIEW1DataGrid
        ...
        Else
            dGRID = Nothing
        End If
        If dGRID Is Nothing Then MsgBox("Please select a DataGrid Row First then execute the command.") : Exit Sub
