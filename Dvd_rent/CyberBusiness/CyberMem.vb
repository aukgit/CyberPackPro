Public Class CyberMem
    ...
    Dim ForStrEvt As Boolean = False
    ...
    Public Sub Load_only_MemberID(ByVal ID As String)
        ADSRC.Twice_Q(Me.Cyber_MemberBindingSource, 0, Nothing, ID)
    End Sub
    ...
    Private Sub Cyber_MonthlyFeesDataGridView_UserAddedRow(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewRowEventArgs) Handles Cyber_MonthlyFeesDataGridView.UserAddedRow
        Dim CxDate1 As Date
        SoftIn.Master_ID_String(Me.Cyber_MonthlyFeesDataGridView, 7)
    ...
    Public Function ChkUnpaid(ByVal date1 As Date)
        ...
        ShowTable.Columns.Add("IP ADDRESS", System.Type.GetType("System.String"), "")
        Dim Nbp As New BindingSource
        Str = ""
        SFC("Month_Name", "Year_of_Month")
        AXQ = "BroadBand"
        AXQ2 = "Cyber"
        If CyberCafe = True Then
            STC(date1.Month, date1.Year)
        Else
            STC(date1.Month, date1.Year)
        End If
        'ExpreC("", "", "n")
        AukF.Db_LoadN(NTbt, "Cyber_MemberID", "Cyber_MonthlyFees")
        Nbp.DataSource = NTbt
        For k As Integer = 0 To Me.AukDatabaseDataSet.Cyber_Member.Rows.Count - 1
            If Me.AukDatabaseDataSet.Cyber_Member.Rows(k).Item("MEMBERCATEGORY").ToString = "" Then
                k += 1
            ElseIf CyberCafe = True And Me.AukDatabaseDataSet.Cyber_Member.Rows(k).Item("MEMBERCATEGORY").ToString <> axq2 Then
                k += 1
            ElseIf CyberCafe = False And Me.AukDatabaseDataSet.Cyber_Member.Rows(k).Item("MEMBERCATEGORY").ToString = axq2 Then
                k += 1
            End If
            ...
            If AukF.BindFind(Nbp, "Cyber_MemberID", nma) = False Then
                ...
                Row.Item(4) = Me.AukDatabaseDataSet.Cyber_Member.Rows(k).Item(2).ToString 'Ip Address
    ...
    Private Sub MemberCategoryComboBox_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MemberCategoryComboBox.SelectedIndexChanged
        On Error Resume Next
        If ForStrEvt = True Then Exit Sub
    ...
    Private Sub PakeList_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles PakeList.SelectedIndexChanged
        ...
        Try
            ...
            'Me.AukDatabaseDataSet.BroadBand_Pakege(Me.PakeList.SelectedIndex).Item("Total_Cost_Per_Month").ToString
            If Me.PakeList.SelectedIndex = 1 Then
                'continue connected prize will be distributed from settings
                Me.Member_Charge_MonthlyTextBox.Text = Val(My.Settings.Cyber_MonthlyFees)
            Else
    ...
    Private Sub PreviousMonthToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles PreviousMonthToolStripMenuItem.Click
        ChkUnpaid((CDate(Now.Date)).AddMonths(-1))
    End Sub
    ...
    Private Sub Before2MonthsToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Before2MonthsToolStripMenuItem.Click
        ChkUnpaid(CDate(Now.Date).AddMonths(-2))
    End Sub
    ...
    Private Sub Cyber_MemberBindingSource_CurrentChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cyber_MemberBindingSource.CurrentChanged
    ...
    Private Sub Timer1_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Timer1.Tick
        If Me.Cyber_MemberBindingSource.Count > 0 Then
            If Me.Cyber_MonthlyFeesBindingSource.AllowNew = False Then
                Me.Cyber_MonthlyFeesBindingSource.AllowNew = True
            End If
        ElseIf Me.Cyber_MemberBindingSource.Count = 0 Then
            If Me.Cyber_MonthlyFeesBindingSource.AllowNew = True Then
                Me.Cyber_MonthlyFeesBindingSource.AllowNew = False
            End If
        End If

        If Me.Cyber_MemberBindingSource.Count = 0 And TDisBool = False Then
            TxBoxDis()
        ElseIf Me.Cyber_MemberBindingSource.Count > 0 And TDisBool = True Then
            TxBoxEna()
        End If
        If Me.Cyber_ConnectCheckBox.Checked = True Then
            Me.Cyber_ConnectCheckBox.Text = "Enabled &Connected"
        Else
            Me.Cyber_ConnectCheckBox.Text = "Disabled &Connected"
        End If
    End Sub
    Private Sub TxBoxDis()
        If TDisBool = False Then

            For K As Integer = 0 To Me.GroupBox1.Controls.Count - 1
                If TypeOf Me.GroupBox1.Controls.Item(K) Is TextBox Then
                    Dim T As TextBox = Me.GroupBox1.Controls.Item(K)
                    T.Enabled = False
                End If
            Next
            TDisBool = True
    ...
    Private Sub Cyber_MonthlyFeesDataGridView_DataError(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewDataErrorEventArgs) Handles Cyber_MonthlyFeesDataGridView.DataError
        GrdErrMsg(e)
    End Sub

    Private Sub Button5_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button5.Click

        Dim s As String = Replace(My.Settings.PingCommand, "[IP]", Me.IP_ADDRESSTextBox.Text)
        AukMod.SavStr(s, AppPath & "\pingCyber.bat")
        AukF.OExe(AppPath & "\pingCyber.bat", Me)
    End Sub

    Private Sub PingContext(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripMenuItem8.Click, ToolStripMenuItem7.Click, ToolStripMenuItem6.Click
        Dim s As String = Replace(sender.text, "[IP]", Me.IP_ADDRESSTextBox.Text)
        AukMod.SavStr(s, AppPath & "\pingCyber.bat")
        AukF.OExe(AppPath & "\pingCyber.bat", Me)
    End Sub

    Private Sub LogOnIPToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LogOnIPToolStripMenuItem.Click
        AukF.OExe("//" & Me.IP_ADDRESSTextBox.Text, Me)
