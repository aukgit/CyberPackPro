...
Public Module SrGens
    ...
    Dim ExpSr As String = "[sname][date][scode][package][cname][mixed]"
    ...
    Public Function MakeSoft_Corrupt(Optional ByVal Category As Integer = 0, Optional ByVal Msg As String = "")
        ...
        If Category = 0 Then
            ...
            My.Computer.Registry.SetValue("HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\ExpSrlorer\AppKey", "app_key_" & SoftCode, xo("883294", 1))
    ...
    Public Function Get_Corrupt(Optional ByVal ExitSoft As Boolean = False, Optional ByVal MSGSH As Boolean = True) As Boolean
        ...
        b = My.Computer.Registry.GetValue("HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\ExpSrlorer\AppKey", "app_key_" & SoftCode, "") ' xo("883294", 1)
    ...
    Public Function CHECK_OTHER_CATEGORY_INS() As Boolean
        ...
        If Str1.Trim <> "" Then
            ...
            If Ua.GetUpperBound(0) < 3 Then
                ...
                Exit Function
            End If
            If Ua.GetUpperBound(0) >= 9 And (Ua.GetValue(0) >= 0 And Ua.GetValue(0) <= Soft_MaxPackage) Then
                Genx = xo(Ua.GetValue(10).ToString.Trim, -5)
                'Ua.SetValue(Genx, 10)
                ExpSr = Ua.GetValue(9).ToString.Trim

                TGen = SrGens.GetGenCode_2(CBool(Ua.GetValue(8).ToString.Trim), Ua.GetValue(1).ToString.Trim, Ua.GetValue(2).ToString.Trim, Ua.GetValue(3).ToString.Trim, Ua.GetValue(4).ToString.Trim, Ua.GetValue(0).ToString.Trim, Ua.GetValue(5).ToString.Trim, )
                'gen = Genx
    ...
    Public Function For_Soft_Combo_HddSr(ByVal Package As String, ByVal Company As String, Optional ByVal CompanyNameMinLen As Integer = 7) As String
        ...
        If HdSerial <> "" Then
            If (Len(Company) >= a) Or a = 0 Then
                ...
                ExpSr = ExpSr.ToLower.Replace("[sname]", SoftName)
                ExpSr = ExpSr.ToLower.Replace("[scode]", SoftCode)
                ExpSr = ExpSr.ToLower.Replace("[package]", Package)
                ExpSr = ExpSr.ToLower.Replace("[company]", Company)
                ExpSr = ExpSr.ToLower.Replace("[gcode]", dm1)
                dm2 = AukF.ConTOAsc(ExpSr, "*", INTO2, True)
                dm4 = AukF.ConTOAsc(ExpSr, "+", Plus, True)
                HashBytes = H.ComputeHash(Uni.GetBytes(ExpSr))
    ...
    'End Function
    Public Function GenSrls_By_DT_Hour(ByVal DateN As String, ByVal PakegeName As String, ByVal CompanyName As String, ByVal serialCodeSum_Plus As Integer, Optional ByVal ExpSr As String = "[sname][date][scode][package][cname][mixed]")
        Dim D1 As Date
        Try
            D1 = CDate(DateN)
        Catch ex As Exception
        ...
        ExpSr = ExpSr.ToLower.Replace("[date]", dat)
        ExpSr = ExpSr.ToLower.Replace("[scode]", SoftCode)
        ExpSr = ExpSr.ToLower.Replace("[package]", PakegeName)
        ExpSr = ExpSr.ToLower.Replace("[cname]", CompanyName)
        ...
        'ExpSr = ExpSr & hrtxt
        ExpSr = ExpSr.ToLower.Replace("[mixed]", hrtxt)
        xn = AukF.ConTOAsc(ExpSr, "*", CDec(a), True)
        If Val(xn) > 0 Then
