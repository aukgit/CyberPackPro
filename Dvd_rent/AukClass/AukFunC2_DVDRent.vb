...
Public Class AukFC
    ...
    Public Function AukBindingSource_RemoveFilter(ByVal Bp As BindingSource)
        ...
        End Try
    End Function
    Public Function BindFind(ByVal Bp As BindingSource, ByVal TitleName As String, ByVal FindTxt As String, Optional ByVal GotoPos As Boolean = False) As Boolean
        Dim Ap As Integer
        On Error GoTo BN
        'Dim D As DataGrid
        ComRow = -1
        ...
        'If DataGridx IsNot Nothing Then
        '    'If SelectNew = True Then
        '    '    If TypeOf (DataGridx) Is DataGrid Then
        '    '    ElseIf TypeOf (DataGridx) Is DataGridView Then
        '    '        DataGridx.ClearSelection()
        '    '    Else
        '    '        MsgBox("DataGrid Object is not Typed", MsgBoxStyle.Critical)
        '    '        'Exit Function
        '    '    End If
        '    'End If
        '    If ComRow > -1 Then
        '        If TypeOf (DataGridx) Is DataGrid Then
        '            DataGridx.Select(ComRow)
        '            Return True
        '        ElseIf TypeOf (DataGridx) Is DataGridView Then
        '            DataGridx.Rows(ComRow).Selected = True
        '            Return True
        '        Else
        '            MsgBox("DataGrid Object is not Typed", MsgBoxStyle.Critical)
        '            Exit Function
...
BN:
        'If DataGridx IsNot Nothing Then
        '    If SelectNew = True Then
        '        If TypeOf (DataGridx) Is DataGridView Then
        '            DataGridx.ClearSelection()
        '        ElseIf TypeOf (DataGridx) Is DataGrid Then
        '        Else
        '            MsgBox("DataGrid Object is not Typed", MsgBoxStyle.Critical)
        '            Exit Function
        '        End If
        '    End If
        'End If
        ComRow = -1 : Return False : Exit Function
    End Function
    ...
    Public Function Single_DataRecordRefresh(ByVal Bp As BindingSource, Optional ByVal Msg As Boolean = False, Optional ByVal MsgTxt As String = "")
        ...
        End Try
    End Function
    Public Function Single_DataRecordRefresh(ByVal D As DataGridView, Optional ByVal Msg As Boolean = False, Optional ByVal MsgTxt As String = "")
    ...
    Public Function Single_DataRecordRefresh(ByVal T As DataTable, ByVal RowI As Integer, Optional ByVal Msg As Boolean = False, Optional ByVal MsgTxt As String = "")
        Try
            If Msg = True Then
                If MsgTxt = "" Then
                    If MsgBox("Do you want to Refresh (" & RowI + 1 & ") Record Data...?", MsgBoxStyle.Exclamation + MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
    ...
    Public Function RejectChg(ByVal TabName As String, ByVal Bp As BindingSource) As Boolean
        Dim D As Object
        D = Bp.DataSource
        If Me.MsgTr(What & "Refresh database(" & TabName & ",Reject Changes)...?") = True Then
            Bp.CancelEdit()
            D.RejectChanges()
        End If
    End Function
    ...
    Public Function GridT(ByVal D As DataGridView, Optional ByVal Column As Integer = -1, Optional ByVal RowIndex As Integer = -1, Optional ByVal DEFAULTSTR As String = "") As Object
        On Error Resume Next
    ...
    Public Function ConTOAsc(ByVal Txt As String, ByVal Expresion As String, ByVal WhichNum As Decimal, ByVal AlsoDoWith As Boolean) As String
        ...
        ConTOAsc = Str
    End Function
    Public Function Prnt(ByVal MRpt As Object, ByVal Dset As Object, Optional ByVal BP As BindingSource = Nothing)
        Dim m As Object
        m = MRpt
...
FINXa:      Bind_WithFilter_AddWith = NewFilter
    ...
    Public Function Grid_View_Advance_SetCommonItems(ByVal D As DataGridView, ByVal Columns As String, ByVal Entry As String, Optional ByVal TrueFalseTxt As String = "", Optional ByVal WithEntry As Boolean = False, Optional ByVal EnrtyBefore As Boolean = False, Optional ByVal AllRecords As Boolean = False, Optional ByVal Prog As ProgressBar = Nothing, Optional ByVal ProgVisibleOpt As Boolean = False, Optional ByVal SetVisible_Opt As Boolean = False)
        ...
        If TrueFalseTxt.Trim <> "" Then
            TrF = True
            Lst3 = TrueFalseTxt.Split(",")
            Array.Resize(Lst3, CInt(Lst.GetUpperBound(0).ToString))
        Else
            TrF = False
        End If
        If Prog IsNot Nothing Then
        ...
        For k As Integer = 0 To Cnt
            ...
            For Im As Integer = 0 To Lst.GetUpperBound(0)
                If TrF = True Then
                    If CheckVal(Lst3.GetValue(Im).ToString) = True Then OCan = True Else OCan = False
                Else
                    OCan = True
                End If
                If OCan = True Then
    ...
    End Function
    Public Function CheckVal(ByVal Str As String) As Boolean
        If Str = 1 Or InStr(Str, "check", CompareMethod.Text) > 0 Or InStr(Str, "tr", CompareMethod.Text) > 0 Or InStr(Str, "yes", CompareMethod.Text) > 0 Then
            CheckVal = True
        Else
            CheckVal = False
        End If
