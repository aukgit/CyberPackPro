...
Public Module AukMod
    ...
    Public Function SavMsg(Optional ByVal MSGTXT As String = Nothing, Optional ByVal TABLEn As String = "", Optional ByVal MST As MsgBoxStyle = MsgBoxStyle.Information, Optional ByVal MsgTypePlus As MsgBoxStyle = MsgBoxStyle.YesNoCancel) As Integer
        ...
        SavInt = SavMsg
    End Function
    Public Function GetMultiSplit_Arrays(ByVal TextCommand As String, ByVal SplitKeys As String, Optional ByVal MainKey As String = "/", Optional ByVal RowKey As String = ",")
        ...
        Dim A() As String 'for handle the single str arrays
        ...
        Dim Chrx() As Char = Chrs1.ToCharArray 'geting them into chr array
        ...
        For K As Integer = 0 To RowAr.GetUpperBound(0) 'eveulate on rows
            ...
            If E1 <> "" AndAlso InStr(E1, ArV(Keys, 0)) > 0 Then
                newAr = E1.Split(Chrx)
                For k1 As Integer = 0 To newAr.GetUpperBound(0) 'first go in childrows rows,
                    If MA(k1) Is Nothing Then
                        MA(k1) = RowAr 'there objects must be like the seris of [main row=RowAr]
                    End If
                    '[child row=newAr]:due=f.lable    -there we have 3 rows cause we split them into chr of split keys
                    'like this
                    'A = MA(k1) 'having the serial of array of 'every rows on there
                    'now A is a array of str. so we can input multi str there bu we have to make 
                    'it  rezie that 
                    'the resize will happen by the [child row=newAr] which we call first on 'rowAr'
                    '[main rows=RowAr]:due=t.lable,func=g.max,lunch=f.kkkk - there is 3 main rows cause rowkey is ','
                    ' every format is same so now we make a resize 
                    If RowAr.Length <> A.Length Then
                        A.Resize(A, newAr.Length)
                    End If
                    'A.SetValue(ArV(newAr, k1), k1) 'insert value on str array by index
                    MA(k1) = newAr
                    'lastly we have to input make ma= a
